...
describe('VesselsService', () => {
  ...
  describe('createVessel', () => {
    ...
    it('validates logic configuration when a logic type is supplied', async () => {
      ...
      logicTypesService.getByNameRaw.mockResolvedValue({
    ...
    it('throws if the referenced logic type cannot be found', async () => {
      logicTypesService.getByNameRaw.mockResolvedValue(null);
  ...
  describe('setAutoControl', () => {
    it('throws when vessel does not exist', async () => {
      vesselRepository.findByPk.mockResolvedValue(null);
    ...
    it('throws when vessel is not configured for auto control', async () => {
      vesselRepository.findByPk.mockResolvedValue({
        id: 'vessel-1',
        type: 'fermenter',
        logicType_id: 'pid',
        logicConfig: { kp: 1 },
  ...
  describe('bootstrapVessels', () => {
    it('queues processing for all auto-enabled vessels', async () => {
      vesselRepository.findAll.mockResolvedValue([
        {
          id: 'v-1',
          logicType_id: 'pid',
          logicConfig: { n: 1 },
  ...
  describe('processLogic', () => {
    it('throws when vessel does not exist', async () => {
      logicTypesService.getByNameRaw.mockResolvedValue({ run: vi.fn() });
      vesselRepository.findOne.mockResolvedValue(null);
    ...
    it('returns early when auto control is disabled', async () => {
      ...
      logicTypesService.getByNameRaw.mockResolvedValue({ run: vi.fn() });
      vesselRepository.findOne.mockResolvedValue({
        id: 'vessel-1',
        autoControlEnabled: false,
      });
    ...
    it('disables auto control and throws when current telemetry is missing', async () => {
      ...
      actorSensorTypesService.getRawActorType
    ...
    it('runs COOL action and queues next state', async () => {
      ...
      logicTypesService.getByNameRaw.mockResolvedValue(logic);
